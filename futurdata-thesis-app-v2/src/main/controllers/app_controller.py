...
class AppController:
    ...
    def _is_shape_connected(self, shape) -> bool:
        """Return True if shape is linked via connection lines or arrow shapes (From THEIR VERSION)."""
    ...
    def _delete_shape(self, shape):
        """Delete a shape, checking for connections first."""
    ...
    def _persist_shape_properties(self, shape):
        """Persist edited shape properties to the backing database when possible (from OUR VERSION)."""
        if isinstance(shape, ComponentBox):
    ...
    def toggle_snap_mode(self):
        """Toggle snap-to-grid from UI binding (from OUR VERSION)."""
    ...
    def zoom_in(self):
        """Zoom in on the canvas (from OUR VERSION)."""
        self.view.canvas.zoom_in()
        self.view.set_status(f"Zoom: {int(self.view.canvas.zoom_factor * 100)}%")

    def zoom_out(self):
        """Zoom out on the canvas (from OUR VERSION)."""
        self.view.canvas.zoom_out()
        self.view.set_status(f"Zoom: {int(self.view.canvas.zoom_factor * 100)}%")

    def reset_zoom(self):
        """Reset the canvas zoom to 100% (from OUR VERSION)."""
    ...
    def show_manage_colors_dialog(self):
        """Open the dialog for managing colors (from OUR VERSION)."""
    ...
    def delete_color(self, color_id: int) -> bool:
        """Deletes a color and refreshes the properties panel if needed (from OUR VERSION)."""
        try:
    ...
    def show_manage_materials_dialog(self):
        """Open the dialog for managing materials (from OUR VERSION)."""
    ...
    def delete_material(self, material_id: int) -> bool:
        """Deletes a material and refreshes the view (from OUR VERSION)."""
        try:
